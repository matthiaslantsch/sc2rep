<div id="mainContainer" class="container">
  ...
  <div class="row">
    <div class="col-sm-10">
      <input type="hidden" id="mapField" value="<?=$match->tagsToArray()["map"]->map->identifier?>"
        data-sizeX="<?=$match->tagsToArray()["map"]->map->sizeX?>"
        data-sizeY="<?=$match->tagsToArray()["map"]->map->sizeY?>">
      <h2>Map: <a target="_blank" href="//wiki.teamliquid.net/starcraft2/<?=$match->tagsToArray()["map"]?>"><?=$match->tagsToArray()["map"]?></a></h2>
      <h2>Players:</h2>
            ...
            <?php foreach($team as $pl): ?>
              <div class="media">
                  ...
                  <?php if($pl->player->bnet != 0): ?>
                    <a href="<?=linkTo("player/{$pl->player->id}")?>">
                      <div class="portrait_border"
                        style="background: url('<?=linkTo("gfx/portrait-summary-".strtolower($pl->currentLeague).".png")?>')
                          no-repeat 50% 50%;">
                ...
                <div class="media-right media-middle">
                  <img src="<?=linkTo("gfx/{$pl->pickedRace}-struct.png")?>" alt="<?=$pl->pickedRace?>">
                </div>
              </div>
            <?php endforeach; ?>
          </div>
      ...
      <h3>Played: <?=date("j M o g:i a", strtotime($match->played))?>, a <?=\holonet\common\readableDurationString($match->length)?> game</h3>
      <hr>
      <h2>Performance:</h2>
        <table class="table">
        <thead>
    ...
    <div class="col-sm-2">
      <a href="<?=linkTo("matches/{$match->id}/download")?>"><button class="btn btn-success">Download</button></a>
      <h2>Tags</h2>
      <?php foreach($match->tagsToArray() as $group => $tag): ?>
        <h4><?=ucfirst($group)?></h4>
        <?php if(is_array($tag)): ?>
          <?php foreach($tag as $tg): ?>
            <a class="tag" href="<?=linkTo("{$tg->group}/{$tg->name}")?>"><?=$tg->name?></a>
          <?php endforeach; ?>
        <?php else: ?>
          <a class="tag" href="<?=linkTo("{$tag->group}/{$tag->name}")?>"><?=$tag->name?></a>
        <?php endif; ?>
      <?php endforeach; ?>
    </div>
  </div>
                  ...
                  <div class="row topBorder dataRow statsRow">
                    ...
                    <div class="col-xs-3">
                      <div class="media">
                        ...
                        <div class="media-right media-middle">
                          <img class="media-object" src="<?=linkTo("gfx/min.png")?>" alt="Minerals">
                        </div>
                      </div>
                    </div>
                    <div class="col-xs-3">
                      <div class="media">
                        ...
                        <div class="media-right media-middle">
                          <img class="media-object" src="<?=linkTo("gfx/{$pl->playedRace}-gas.png")?>" alt="Vespene gas">
                        </div>
                      </div>
                    </div>
                    <div class="col-xs-3">
                  ...
                  <div class="row bottomBorder dataRow">
                    ...
                    <div class="col-xs-3">
                      <div class="media">
                        ...
                        <div class="media-right media-middle">
                          <img class="media-object" src="<?=linkTo("gfx/{$pl->playedRace}-food.png")?>" alt="Supply">
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  <div class="container">
    <div id="tabs" class="tabs">
      <ul>
        ...
        <li><a href="<?=linkTo("matches/{$match->id}/msg")?>">Chat history</a></li>
        <?php foreach($match->performances as $perf): ?>
          <li>
            <a href="<?=linkTo("matches/{$match->id}/advStats_{$perf->sid}")?>">
